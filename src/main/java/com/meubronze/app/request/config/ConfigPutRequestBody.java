...
@AllArgsConstructor
@NoArgsConstructor
@ConfigPutRequestBody.ValidTimeSum
public class ConfigPutRequestBody {
    @NotNull
    @Min(value = 1, message = "id starts from 1")
    private Long id;
    ...
    public static class TimeSumValidator implements ConstraintValidator<ValidTimeSum, ConfigPutRequestBody> {
        @Override
        public boolean isValid(ConfigPutRequestBody configPutRequestBody, ConstraintValidatorContext context) {
            return configPutRequestBody.getDefaultHours() + configPutRequestBody.getDefaultMins() + configPutRequestBody.getDefaultSecs() > 0;
        }
    }
}
